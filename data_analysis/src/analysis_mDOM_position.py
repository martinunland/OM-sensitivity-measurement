...
from src.helper import (
    create_laser_mask,
    create_laser_mask_interp,
    get_likely_laser_pulses,
)
...
class FilteredData:
    ...
    def summarise_data_and_clean(self):
        ...
        - Signal wait times (laser periods)
        - Background time differences
        - Phase deviations
        - Signal and background charge distributions

        Cleans raw data to free memory after processing."""
        wait_time = np.diff(np.sort(self.signal_timestamps)) / (1 / self.laser_freq)
        self.signal_h, bins = np.histogram(wait_time, bins=np.arange(0.5, 80 + 0.5, 1))
        ...
        self.background_h, bins = np.histogram(
            np.log10(background_dt), bins="sqrt", range=(-4.0, -1.5)
        )
        self.background_x = edge_to_x(bins)

        self.deviation_h, bins = np.histogram(
            self.deviation, bins=200, range=(-10e-6, 10e-6)
        )
        ...
        self.signal_charge_h, bins = np.histogram(
            self.signal_charges, bins="sqrt", range=(0, 5)
        )
        self.signal_charge_x = edge_to_x(bins)

        self.background_charge_h, bins = np.histogram(
            self.background_charges, bins="sqrt", range=(0, 5)
        )
...
class PositionStage:
    """Processes data from a single mDOM/fiber position stage.
    
    Handles data for all wavelengths and PMTs at a specific geometric position.
    Manages laser phase tracking and signal/background separation.
    """

    def __init__(self):
        self.wavelength_data_file: Dict[float, SingleFileProcessor] = (
            dict()
        )  # All data from files, will be later filtered and cleaned
        self.mean_ref_current: Dict[float, uncertainties.core.Variable] = (
            dict()
        )  # Mean reference current for each wavelength
        self.PMT_data: Dict[int, Dict[float, FilteredData]] = {
            i: {} for i in range(24)
        }  # Filtered data for each channel and wavelength
        self.set_laser_freq()
    ...
    def add_single_file_data(self, wavelength: float, data_file: SingleFileProcessor):
        """Adds processed data file for a specific wavelength."""
        self.wavelength_data_file[wavelength] = data_file

    def add_reference_current(
        self, wavelength: float, current: uncertainties.core.Variable
    ):
        """Adds reference photodiode current for a wavelength."""
        self.mean_ref_current[wavelength] = current

    def process_stage(self):
        """Process all data for this position stage."""
        self.calculate_laser_phase()
        self.filter_data_and_clean()

    def calculate_laser_phase(self):
        """Determines laser phase using all available data except 400nm."""
        ...
        for wavelength, data_file in self.wavelength_data_file.items():
            if (
                wavelength == 400
            ):  # There is no light from the laser at 400 nm, so we cannot use it for the laser phase
                continue
            for chunk in data_file.data_chunks:
                likely_pulses = []
                for channel_chunk in chunk.values():
                    likely_pulses.extend(
                        get_likely_laser_pulses(
                            channel_chunk.timestamp, self.laser_freq
                        )
                    )
    ...
    def filter_data_and_clean(self):
        """Separates signal/background for all channels and wavelengths."""
        for wavelength, data_file in self.wavelength_data_file.items():
            for chunk in data_file.data_chunks:
    ...
    def _filter_channel_data(
        self, channel: int, wavelength: float, ch_chunk: ChannelChunkData
    ):
        """Processes data chunk for a specific channel and wavelength.
        Creates signal/background separation using fixed and interpolated phases,
        choosing the better performing method.
        """
        ...
        mask1, deviation1, mean_phase1 = create_laser_mask(
            ch_chunk.timestamp, self.mean_phase, self.laser_freq, self.time_window
        )
        mask2, deviation2, mean_phase2 = create_laser_mask_interp(
            ch_chunk.timestamp,
            self.phase_interpolator,
            self.laser_freq,
            self.time_window,
        )
