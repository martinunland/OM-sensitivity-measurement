...
class Fit:
    """Fits signal and background models to PMT data using maximum likelihood."""
    ...
    def set_passing_fractions(self, passing_fractions: List[float]):
        """Sets efficiency factors for different photon numbers."""
        self._passing_fractions = passing_fractions

    def _calculate_factorials(self):
        """Pre-calculates log-factorials for likelihood computation."""
        log_factorial = []
        for y in self.y_l:
    ...
    def _P_no_background_in_tw(self, dark_rate, t_w):
        """Probability of no background events in time window."""
        mu_b = dark_rate * t_w
        return np.exp(-mu_b)

    def _P_dead(self, mu, dark_rate, norm_l, norm_b):
        """DEPRECATED: Probability of detector dead time from previous event."""
    ...
    def _P_no_photons_from_laser(self, mu):
        """Probability of no detected photons from laser flash.
        Accounts for multi-photon states and detection efficiencies renormalising Poisson probabilities.
        """
        P_0p = np.exp(-mu)
        P_ip = []
    ...
    def _P_measure_nothing(self, mu, dark_rate, norm_l, norm_b):
        """Probability of no signal detection in a laser window."""

        # After thinking about it, there should be no dead time in the laser window. The window is much larger than the dead time (500ns).
    ...
    def time_distribution_model(self, mu, dark_rate, norm_l, norm_b):
        """Model for Delta_t distribution of detected signals."""
    ...
    def background_model(self, mu, dark_rate, norm_l, norm_b):
        """Model for background Delta_t distribution."""
    ...
    def llh(self, mdl, y, factorial):
        """Computes log-likelihood for Poisson-distributed data."""
        return np.sum(-mdl + np.log(mdl) * y - factorial)

    def poisson_llh(self, mu, dark_rate, norm_l, norm_b):
        """Combined log-likelihood for signal and background data."""
...
def fit_filtered_data(
    ...
    """Performs maximum likelihood fit on filtered PMT data.
    
    Returns:
        mu, mu_err, dark_rate, dark_rate_err, goodness_of_fit
        Returns zeros and -1 if fit fails
    """
